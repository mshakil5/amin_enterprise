...
<script>
 $(document).ready(function() {
    ...
    $('#select_all').on('click', function() {
        var rows = equityTBL.rows({ 'search': 'applied' }).nodes();
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.status === 200) {
                    ...
                    equityTBL.ajax.reload(null, false); // Reload table without resetting page
                } else {
    ...
    $('#equityTBL').on('click', '.undo-single-btn', function(e) {
        e.preventDefault();
        var id = $(this).data('id');
