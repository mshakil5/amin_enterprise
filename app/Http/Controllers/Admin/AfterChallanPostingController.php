...
{
    // Increase execution time for large datasets (10 mins)
    set_time_limit(600); 
    ...
    $updatedCount = 0;
    $failedRecords = []; // Track which IDs failed

    // Process in server-side chunks of 500
    ProgramDetail::with('challanRate', 'program')
        ->whereBetween('date', [$fromdate, $todate])
        ->whereNotNull('headerid')
        ->where('client_id', $clientId)
        ->chunk(500, function($programDetails) use (&$updatedCount, &$failedRecords) {
            
            foreach ($programDetails as $detail) {
                
                // Wrap individual record in try-catch and transaction
                try {
                    DB::transaction(function() use ($detail, &$updatedCount) {
                        $cQty = $detail->challanRate->sum('qty');
                        if ($cQty <= 0) return;
                        ...
                        if (!empty($rowsToInsert)) {
                            ...
                            // Prepare logs in an array for faster batch insertion
                            ...
                            if (!empty($logsToInsert)) {
                                ChallanRateLog::insert($logsToInsert);
                            }

                            // 4. Update Program Detail
                    ...
                    // If this specific record fails, log the error and save the ID
                    Log::error("Bulk Update Failed for ProgramDetail ID {$detail->id}: " . $e->getMessage());
                    $failedRecords[] = $detail->id;
                    
                    // Continue to the next record without stopping the loop
                    continue; 
                }
            }
        });
    ...
    if (count($failedRecords) > 0) {
        $message .= ' ' . count($failedRecords) . ' records failed and were skipped.';
    }
